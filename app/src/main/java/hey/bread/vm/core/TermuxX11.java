...
public class TermuxX11 {
    ...
    public static void main(String[] args) throws ErrnoException {
        File xkbConfigRoot = new File(BreadVmApplication.getContext().getFilesDir().getAbsolutePath() + "/usr/share/X11/xkb");
        if (xkbConfigRoot.exists()) {
            Os.setenv("XKB_CONFIG_ROOT", xkbConfigRoot.getAbsolutePath(), true);
            File tmpDir = new File(BreadVmApplication.getContext().getFilesDir().getAbsolutePath() + "/usr/tmp");
            FileUtils.delete(tmpDir.getAbsolutePath());
            tmpDir.mkdirs();
            Os.setenv("TMPDIR", tmpDir.toString(), true);
            try {
                PackageInfo targetInfo = BreadVmApplication.getContext().getPackageManager().getPackageInfo(TARGET_APP_ID, 0);
