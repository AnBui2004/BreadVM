...
public class LoggerDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        LogsAdapter mLogAdapter = new LogsAdapter(layoutManager, BreadVmApplication.getApp(), forceLightText);
                        ...
                        try {
                            ...
                            requireActivity().runOnUiThread(() -> {
                                if (!finalLogLine.isEmpty())
                                    BreadStatus.logError("<font color='red'>[E] " + finalLogLine + "</font>");
                                if (!finalLogLine1.isEmpty())
                                    BreadStatus.logError("<font color='#FFC107'>[W] " + finalLogLine1 + "</font>");
                                isReading = false;
                            });
                        } catch (IOException e) {
