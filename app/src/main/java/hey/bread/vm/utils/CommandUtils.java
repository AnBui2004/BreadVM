...
public class CommandUtils {
    ...
    public static String getQemuVersion(Context context) {
        return BreadVmApplication.getContext() == null ? "Unknow" : new Terminal2(context).executeOnThisThread("qemu-system-x86_64 --version | head -n1 | awk '{print $4}'").replaceAll("\n", "");
    }

    public static boolean is3dfxVersion(Context context) {
        return BreadVmApplication.getContext() != null && new Terminal2(context).executeOnThisThread("qemu-system-x86_64 --version").contains("3dfx");
    }
}
