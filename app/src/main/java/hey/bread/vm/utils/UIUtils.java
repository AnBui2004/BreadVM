...
import hey.bread.qemu.utils.FileUtils;
...
public class UIUtils {
    ...
    public static Spannable formatAndroidLog(String contents) {
        ...
        try {
            ...
            Log.e(TAG, "Could not format bread log: " + ex.getMessage());
        } finally {
            if(scanner!=null) {
                try {
                    scanner.close();
	...
	public static void toastLong(final Context activity, final String errStr) {
		new Handler(Looper.getMainLooper()).post(new Runnable() {
			...
			public void run() {
				...
				toast.show();
                BreadStatus.logInfo("<font color='#009688'>[I] "+errStr+"</font>");
        ...
        alertDialog.setButton(DialogInterface.BUTTON_POSITIVE, "Yes",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        FileUtils.viewBreadVMLog(activity);
                    }
                });
