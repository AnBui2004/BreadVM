...
## 🔃 Upgrade tool
Upgrade or change the Qemu version you want to use without resetting the Bread VM using the Terminal.

QEMU 11.0.0 (requires Bread VM 4.0.8+):
```bash
...
QEMU 9.2.4 - 3dfx (requires Bread VM 3.5.0+):
```bash
...
QEMU 9.2.2 - 3dfx (recommended and for Bread VM 4.1.1+):
```bash
...
QEMU 7.2.22 - 3dfx (requires Bread VM 4.2.2+):
```bash
...
## ⚙️ Bootstraps
QEMU 11.0.0 (for Bread VM 4.0.8+):
...
QEMU 9.2.4 - 3dfx (only for Bread VM 3.5.0):
...
QEMU 9.2.2 - 3dfx (recommended and for Bread VM 4.4.7+):
...
QEMU 9.2.2 - 3dfx (recommended and for Bread VM 3.5.1+):
- [For Android ARM (64-bit)](https://github.com/AnBui2004/Vectras-VM-Emu-Android/releases/download/3.5.1/base-genegic-nosve-vectras-vm-arm64-v8a.tar.gz)
...
QEMU 9.2.2 - 3dfx (for Bread VM 3.2.9 - 3.4.9):
...
QEMU 8.2.0 - 3dfx (only for Bread VM 2.9.5):
...
QEMU 7.2.22 - 3dfx (for Bread VM 4.4.7+):
...
QEMU 7.2.22 - 3dfx (for Bread VM 4.2.2+):
